...
def get_broker_config(
    ...
    for broker in inputted_brokers:
        ...
        supported_brokers = ["oanda", "ib", "ccxt", "virtual", "quantumhedge"]
        ...
        elif broker.lower() == "quantumhedge":
            api_key = "LIVE" if environment.lower() == "live" else "PRACTICE"
            quantumhedge_conf = global_config["QUANTUMHEDGE"]

            # Initialise config dict
            config = {"PORT": quantumhedge_conf["PORT"]}

            config["LISENCE"] = quantumhedge_conf["LISENCE"]

            config["ACCOUNT_ID"] = quantumhedge_conf["ACCOUNT_ID"]
...
class RealTimeDataStream(DataStream):
    ...
    def __init__(self, config: dict[str, any]) -> None:
        ...
        #self._data_stream_object= config["datastreamer"]
...
###此处为LZC添加
"""class RealtimeStream(LocalDataStream):

    def get_candles(self, instrument: str, *args, **kwargs) -> pd.DataFrame:
        # 调用父类方法加载原始数据
        df = super().get_candles(instrument, *args, **kwargs)
        print("df",df)
        # 模拟实时流：仅返回最后一行（最新数据）
        latest_candle = df.iloc[-1:].copy()
        print("lastest_candle",latest_candle)
