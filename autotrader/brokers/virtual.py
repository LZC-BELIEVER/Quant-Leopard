...
class Broker(AbstractBroker):
    ...
    def configure(
        ...
        **kwargs,
        ...
        print("initialising virtual data_broker")
    ...
    def get_candles(
        ...
        **kwargs,
        ...
        if instrument in self._data_cache:
            ...
            #print("enter virual get_candles!")
            #print("vitual self.data_broker",self.data_broker)
            #print("vitual self._data_broker",self._data_broker)
        ...
        if self._paper_trading:
            # Do not need to do a time check
    ...
    def _initialise_data(
        ...
        if instrument not in self._data_cache:
            print("inise_data,instrument",instrument)
            candles = self.data_broker.get_candles(
                instrument=instrument,
                granularity=granularity,
    ...
    def _update_positions(
        ...
        def stop_trigger_condition(order_stop_price, order_direction) -> bool:
            ...
            #print("low,high,stop",candle["Low"],candle["High"],order_stop_price)
            low = float(candle["Low"])  # numpy.int64 → float
            high = float(candle["High"])  # numpy.int64 → float
            stop_price = float(order_stop_price)  # decimal.Decimal → float
            triggered = low < stop_price < high
