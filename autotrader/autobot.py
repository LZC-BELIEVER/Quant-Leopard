...
class AutoTraderBot:
    ...
    def start_market_status_timer(self):
        """启动市场状态定时器（作为类方法使用），主进程结束时自动停止"""

        def timer_loop():
            target_times = [(14, 57), (22, 57)]  # 目标时间 (小时, 分钟)

            while True:  # 无需检查 _timer_running，依赖主进程退出自动终止
                now = datetime.now()
                ...
                if weekday < 5:  # 周一到周五
                    for target_hour, target_min in target_times:
                        if now.hour == target_hour and now.minute == target_min:
                            try:
                                self._broker.api.sse_client.is_mkt_open = False
                                print(f"[{now.strftime('%Y-%m-%d %H:%M:%S')}] 市场状态已设置为关闭")
                            except AttributeError:
                                print("警告: broker对象结构不符合预期")

                            time.sleep(60)  # 避免重复设置
                else:
    ...
    def _market_monitor(self):
        """后台线程，最多检测20次，每秒一次"""
        for i in range(5):
            if self._broker.api.sse_client:
                #if self._broker.api.sse_client.is_logged_in:
                if not self._broker.api.sse_client.is_mkt_open:
                    print("检测到市场关闭，准备退出程序。")
                    time.sleep(10)
                    self._broker.clear_positions(self.instrument)
                    print("positions cleared")
                    time.sleep(8)
                    os._exit(0)
            time.sleep(0.1)  # 每秒检测一次
        now_str = datetime.now().strftime("%Y-%m-%d %H:%M")
        print(f"[{now_str}] 未发现市场关闭，停止检测，程序继续运行。")
    ...
    def _update(self, timestamp: datetime) -> None:
        ...
        '''if not hasattr(self, '_monitor_thread') or not self._monitor_thread.is_alive():
            self._monitor_thread = threading.Thread(target=self._market_monitor, daemon=True)
